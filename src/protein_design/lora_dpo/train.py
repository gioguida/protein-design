"""Hydra-driven LoRA-DPO training entrypoint.
...
from protein_design.dpo.loss import batch_monitoring_metrics, dpo_loss, weighted_dpo_loss
...
from protein_design.dpo.utils import (
    build_full_run_name,
...
class _ReferenceView:
    """Reference scorer using the same base model with LoRA adapters disabled."""

    def __init__(self, lora_model: ESM2Model) -> None:
        self._model = lora_model

    def _run_with_adapters_disabled(self, fn: Any, *args: Any, **kwargs: Any) -> Any:
        kwargs["use_grad"] = False
        disable_result = self._model.model.disable_adapter_layers()
        if hasattr(disable_result, "__enter__") and hasattr(disable_result, "__exit__"):
            with disable_result:
                return fn(*args, **kwargs)
        try:
            return fn(*args, **kwargs)
        finally:
            self._model.model.enable_adapter_layers()

    def masked_pseudo_log_likelihood(self, *args: Any, **kwargs: Any) -> torch.Tensor:
        return self._run_with_adapters_disabled(
            self._model.masked_pseudo_log_likelihood,
            *args,
            **kwargs,
        )

    def pseudo_log_likelihood(self, *args: Any, **kwargs: Any) -> torch.Tensor:
        return self._run_with_adapters_disabled(
            self._model.pseudo_log_likelihood,
            *args,
            **kwargs,
        )
...
def _build_scorers(cfg: Any, logger: logging.Logger) -> ESM2Model:
    ...
    if init_checkpoint is not None:
        ...
        logger.info("Initialized from checkpoint: %s", init_checkpoint)
    ...
    param_info = model.param_summary()
    logger.info(
        "LoRA model: %d trainable / %d total (%.2f%%)",
        param_info["trainable"],
        param_info["total"],
        100.0 * param_info["trainable"] / max(1, param_info["total"]),
    )
...
def _save_checkpoint(
    ...
    adapter_state_dict = {
        ...
        if "lora_" in key or "lora_embedding" in key
    }
    state = {
        "epoch": int(epoch),
...
def _load_checkpoint(
    ...
    ckpt = torch.load(checkpoint_path, map_location="cpu")
    adapter_sd = ckpt.get("adapter_state_dict", {})
    if adapter_sd:
...
def run_lora_dpo(cfg: Any) -> Path:
    ...
    logger.info("Starting LoRA-DPO training run")
...
def main(cfg: Any) -> None:
    ...
    if runner != "lora_dpo":
        raise ValueError(
            "LoRA-DPO entrypoint requires a LoRA-DPO task preset. "
            "Run with `task=lora_dpo`."
        )
    run_lora_dpo(cfg)
...
if __name__ == "__main__":
    ...
    if all(not arg.startswith("task=") for arg in sys.argv[1:]):
        sys.argv.insert(1, "task=lora_dpo")
    main()
